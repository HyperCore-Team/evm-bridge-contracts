...
async function unwrap() {
    ...
    let unwrapGas = await bridge.connect(user1).estimateGas.unwrap(constants.wZNNAddress, unwrapAmount, alphanetAddress, ovUnwrap);
    let unwrapCost = unwrapGas.mul(gasPrice);
    if(unwrapCost.lt(bnbBalance)) {
        ovUnwrap.gasLimit = unwrapGas;
        let unwrapTx = await bridge.connect(user1).unwrap(constants.wZNNAddress, unwrapAmount, alphanetAddress, ovUnwrap);
        await unwrapTx.wait();
